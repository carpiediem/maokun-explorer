export default function highlightPath(code, paths, places) {
  const { landmarks } = paths.find(
    (p) => p.properties.code === code
  ).properties;

  Array.from(document.querySelectorAll(`path.code-${code}`)).forEach((p) =>
    p.classList.add('selected')
  );

  landmarks.forEach((id) => {
    const maokunLandmark = document.querySelector(
      `section.maokun path.circle-marker.id-${id}`
    );
    const modernLandmark = document.querySelector(
