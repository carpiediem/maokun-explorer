...
const hasKnownLocation = (f) =>
  ...
  (f.geometry.type === 'LineString' && f.properties.code.length && f.geometry.coordinates);
...
const ModernMap = forwardRef((props, ref) => {
  return (
    <section className="modern">
      ...
      >
        <TileLayer attribution={TILE_ATTRIBUTION} url={TILE_SOURCE} prefix={false} maxZoom={20} />
        <ZoomControl position="bottomright" />
        {props.places.filter(hasKnownLocation).map((f) => (
          <LabeledMarker key={f.properties.id} labeled={props.labelLocations} onSelect={props.onSelect} {...f} />
        ))}

        {props.paths.filter(hasKnownLocation).map(({ properties, geometry }) => (
          <Polyline
            key={properties.code}
