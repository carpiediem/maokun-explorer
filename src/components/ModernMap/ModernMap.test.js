...
const KNOWN_POINT = {
  ...
  geometry: { type: 'Point', coordinates: [75.75, 11.25], zoomify: [100, 200] },
};

const UNKNOWN_POINT = {
  type: 'Feature',
  properties: { id: 2, category: 'town', nameEn: 'Equator Point', nameTc: '' },
  geometry: { type: 'Point', coordinates: [0, 10], zoomify: [] },
};
...
test('only renders markers/paths with a known location', () => {
  render(
    <ModernMap
...
test('renders the tile layer and zoom control', () => {
  render(<ModernMap places={[]} paths={[]} labelLocations={false} onSelect={jest.fn()} />, intlEnWrapper);
...
test('clicking a marker selects the point', () => {
  ...
  render(<ModernMap places={[KNOWN_POINT]} paths={[]} labelLocations={false} onSelect={onSelect} />, intlEnWrapper);

  userEvent.click(screen.getByTestId('circle-marker'));
  expect(onSelect).toHaveBeenCalledWith(1, 'point', 'modern');
});

test('clicking a path selects the path', () => {
  const onSelect = jest.fn();
  render(<ModernMap places={[]} paths={[KNOWN_PATH]} labelLocations={false} onSelect={onSelect} />, intlEnWrapper);

  userEvent.click(screen.getByTestId('polyline'));
  expect(onSelect).toHaveBeenCalledWith('wei-1', 'path', 'modern');
});
...
test('clicking the base map deselects', () => {
  ...
  render(
    <ModernMap places={[]} paths={[]} labelLocations={false} onSelect={onSelect} onClick={onClick} />,
    intlEnWrapper,
  );
